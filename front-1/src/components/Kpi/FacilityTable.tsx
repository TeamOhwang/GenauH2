...
export default function FacilityTable({ data = [], start, end }: Props) {
  const columns: ColumnDef<FacilityKpi>[] = [
    ...
    },
    {
      ...
      cell: (info) => (
        <CountUp end={info.getValue<number>() ?? 0} decimals={1} duration={0.6} />
      ),
    },
    {
      header: "실제생산(kg)",
  ...
  return (
    <div
      className="
        text-sm h-full flex flex-col p-4 rounded-lg shadow-md
        bg-white text-black
              ...
              <tr key={hg.id}>
                {hg.headers.map((header) => (
                  <th
                    key={header.id}
                    className="
                      px-3 py-2 border border-gray-300 dark:border-slate-600
              ...
              >
                {row.getVisibleCells().map((cell) => (
                  <td
                    key={cell.id}
                    className="
                      px-3 py-2 border border-gray-200 dark:border-slate-800
